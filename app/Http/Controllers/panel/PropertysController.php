...
class PropertysController extends Controller
{
    ...
    public function contact()
    {
        ...
        if (Session::get('a_id') != null && Session::get('a_id') != "") {
    ...
    public function location(Request $request)
    {
        ...
        if (isset($_GET['stnfet'])) {


            $subscriptiontype = $_GET['stnfet'];

            if (Session::get('property_id')) {
                Property::where('id', Session::get('property_id'))
                    ->update(['subcription_type' => $subscriptiontype]);
            }

            Session::put('subcription_type', $_GET['stnfet']);
        }
        if ($request->has('slot')) {


            Session::put('selected_slot', $request->slot);

            if (Session::get('property_id')) {
                Property::where('id', Session::get('property_id'))
                    ->update(['subcription_type' => $request->slot]);
            }
        }
        if (Session::get('property_id') == "") {
            ...
            if (isset($_GET['edit_id'])) {
                ...
                $subscriptiontype = Session::get('stnfet');
            ...
            if (isset($main_cat)) {
                ...
                if ($ave) {
                    ...
                    Session::put('subcription_type', $subscriptiontype);
                }
            }
        }
        ...
        if (isset($_GET['id'])) {
            ...
            if ($_GET['id'] == 2) {
                ...
                if (Session::get('property_id') != "") {
                    ...
                    //$data->subcription_type=$subscriptiontype;
                    if ($main_cat == 9 || $main_cat == 10 || $main_cat == 11 || $main_cat == 12) {
                        $data->subcription_type = $subscriptiontype;
                    }
                    $data->save();
    ...
    public function price(Request $request)
    {
        ...
        if (Session::get('property_id') != "") {
            ...
            //   
            //   dd($property->main_cat);
            if ($property->main_cat == '7' && $request->id == 2) {
                // dd($request->id);

                return view('panel.property.location_manually', compact('data', 'county'));

            } elseif ($property->main_cat == '7') {
...
//        }
//      if (isset($_GET['fa'])) {
//             $property->facilities = implode(",", $_GET['fa']);
//         }

    //         if (isset($_GET['fa1'])) {
//             $property->facilities = implode(",", $_GET['fa1']);
//         }
...
// }
    public function details_store()
    {
        if (Session::get('property_id') != "") {
            ...
            // --------------------------------------------
            // FIXED FACILITIES fa / fa1
            // --------------------------------------------
            if (isset($_GET['fa']) && is_array($_GET['fa'])) {
                $property->facilities = implode(",", $_GET['fa']);
            }

            if (isset($_GET['fa1']) && is_array($_GET['fa1'])) {
                $property->facilities = implode(",", $_GET['fa1']);
            }
            // --------------------------------------------
            ...
            if ($property_save) {
                ...
                // --------------------------------------------
                // FETCH ALL FACILITIES + SELECTED FACILITIES
                // --------------------------------------------
                $mainCat = Session::get('main_cat');

                // Fetch facilities where the current main category is in the comma-separated category_ids list
                $facilities = Facility::whereRaw("FIND_IN_SET(?, category_ids)", [$mainCat])->get();
                ...
                if (Session::get('main_cat') == 5) {
                    return view(
                        'panel.property.Facilities_sale',
                        compact('property', 'facilities', 'selectedFacilities')
                    );
                }

                if (Session::get('main_cat') == 4 || Session::get('main_cat') == 7 || Session::get('main_cat') == 30 || Session::get('main_cat') == 12) {
                    return redirect()->route('description');
                }

                // if(Session::get('main_cat')==12){
                //     return view(
                //         )
                // }

                return view(
                    'panel.property.Facilities',
                    compact('property', 'facilities', 'selectedFacilities')
                );
            }
        }
...
// }
    public function Facilities()
    {
        if (Session::get('property_id') != "") {
            ...
            // Get facilities based on property category
            $main_cat = Session::get('main_cat');

            // 3: Commercial Rent, 6: Commercial Sale, 11: Commercial needed
            if (in_array($main_cat, [3, 6, 11])) {
                $facilities = Facility::whereIn('type', ['commercial', 'all'])->get();
            } else {
                // Residential by default (1, 2, 5, 8, 9, 10, etc.)
                $facilities = Facility::whereIn('type', ['residential', 'all'])->get();
            }

            // Get selected facilities of this property
            $selectedFacilities = PropertyFacility::where('property_id', $property_id)
                ->pluck('facility_id')
                ->toArray();
    ...
    // add saveFacilities method if not present (example)
    public function saveFacilities(Request $request)
    {
        $property_id = $request->input('property_id');
        if (!$property_id) {
            return back()->with('error', 'Property ID missing.');
        }

        // remove old selections
        PropertyFacility::where('property_id', $property_id)->delete();

        // insert new ones
        if ($request->has('facility_ids') && is_array($request->facility_ids)) {
            $inserts = [];
            foreach ($request->facility_ids as $fid) {
                $inserts[] = [
            ...
            if (!empty($inserts)) {
                PropertyFacility::insert($inserts);
            }
        }

        return back()->with('success', 'Facilities updated successfully.');
    }
...
//         $property = Property::find($property_id);
    ...
    //         // ✅ Get all facilities from table
//         $facilities = Facility::all();

    //         if(Session::get('main_cat') == 5)
//         {
//             return view('panel.property.Facilities_sale', compact('property', 'facilities'));
//         }
//         else
//         {
//             return view('panel.property.Facilities', compact('property', 'facilities'));
//         }
//     }
// }
    ...
    public function pay_needed(Request $request)
    {
        $PropertySubscription = PropertySubscription::all();

        // return view('panel.property.pay_needed',compact('PropertySubscription'));
        return view('panel.property.pay_needed', [
            'PropertySubscription' => $PropertySubscription,
            'id' => $request->query('id'),
            'main_cat' => $request->query('main_cat'),
    ...
    public function display_property()
    {
        ...
        //   dd($property);
        $main_Cat = DB::table('main_category')->whereNotIn('id', [9, 10, 11, 12, 13])->get();
    ...
    public function upgrade_featured()
    {
        ...
        $id = Session::get('property_id');
        $property = Property::find($id);
        if ($property->subcription_type == 2) {
            $property->subcription_type = 1;
        } elseif ($property->subcription_type == 3) {
            $property->subcription_type = 4;
        } elseif ($property->subcription_type == 11) {
            $property->subcription_type = 12;
        } elseif ($property->subcription_type == 13) {
            $property->subcription_type = 14;
        } elseif ($property->subcription_type == 15) {
            $property->subcription_type = 16;
        } elseif ($property->subcription_type == 17) {
            $property->subcription_type = 18;
        } else {
            $property->subcription_type = 1;
        }
        //$property->subcription_type=1;
