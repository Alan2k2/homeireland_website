...
<!--=========================MAIN STARTS======================================-->
                      <main style="border: 0px solid rgb(221, 221, 221);padding:10px;font-size:15px">
                          
                           <div class="row">
                               <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                
                                <center> <h1 style="color:red"><b>Enquiries</b></h1></center>
      ...
      <thead>
        <tr>
        <th class="enq-col" scope="col">SI.NO</th>
        <th class="enq-col" scope="col">Name</th>
        <th class="enq-col" scope="col">Phone</th>
        <th class="enq-col" scope="col">Email</th>
        <th class="enq-col" scope="col">Message</th>
        <th class="enq-col" scope="col">Delete</th>
        </tr>
      </thead>
      <tbody>
        @forelse($enquiries as $slot)
        @php
    $fw = 200;
      @endphp
        ...
        @php
    $fw = 800;
    DB::table('property_enquiries')->where('id', $slot->id)->update(['view_user' => 0]);
      @endphp
      @endif
        <tr style="font-weight:{{$fw}}">
        <td>{{$enquiries->firstItem() + $loop->index }}</td>
        <td>{{$slot->name}}</td>
        <td>{{$slot->phone}}</td>
        <!--<td><a href="mailto:{{$slot->email}}"></a></td>-->
        <td><a href="mailto:{{$slot->email}}" title="replay">{{$slot->email}} &nbsp;&nbsp; <i class="fa-solid fa-reply"></i></a></td>
        <td>{{$slot->message}}</td>
        <td style="color:red">
        <a href="{{url('delete_Enquiries')}}?id={{$slot->id}}"
      ...
      <p style="color:red">No Enquiries.</p>
      </td>
      </tr>
    @endforelse
      </tbody>
   ...
   <div class="d-block d-md-none">
    @forelse($enquiries as $slot)
        <div class="card mb-3 position-relative">
            <a href="{{url('delete_Enquiries')}}?id={{$slot->id}}" 
               class="btn btn-danger btn-sm delete-icon" 
               onclick="return confirm('Are you sure you want to delete this item?');">
                <i class="fa fa-trash" aria-hidden="true"></i>
            </a>
            <div class="card-body">
                <h5 class="card-title">Enquiry #{{$enquiries->firstItem() + $loop->index}}</h5>
                <p class="card-text"><strong>Name:</strong> {{$slot->name}}</p>
                <p class="card-text"><strong>Phone:</strong> {{$slot->phone}}</p>
                <p class="card-text"><strong>Email:</strong> <a href="mailto:{{$slot->email}}" style="text-decoration:none;">{{$slot->email}} &nbsp;&nbsp; <i class="fa-solid fa-reply"></i></a></p>
                <p class="card-text"><strong>Message:</strong> {{$slot->message}}</p>
            </div>
        </div>
    @empty
